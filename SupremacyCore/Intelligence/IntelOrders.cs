...
namespace Supremacy.Intelligence
{
    ...
    public class IntelOrders : IEnumerable<IntelOrder>
    {
        ...
        //{
        //    public readonly Dictionary<int, string> _localIntelOrders; // Dictionary, int key is ownerID & order from enum above this IntelOrders class
        //    public SetNewIntelOrders(int attackingCivID, string _intelOrder)
        //    {
        //        if (attackingCivID < 0 || attackingCivID > 6)
        //            throw new ArgumentNullException("source");
        //        GameLog.Core.Intel.DebugFormat("Set order = {1} for attacker {0}", attackingCivID, _intelOrder.ToString());
        //        _localIntelOrders.Add(attackingCivID, _intelOrder.ToString());

        //        foreach (var item in _localIntelOrders)
        //        {
        //            GameLog.Core.Intel.DebugFormat("_localIntelOrders: civ = {0}, _intelOrder = {1}", item.Key, item.Value.ToString());
        //        }
