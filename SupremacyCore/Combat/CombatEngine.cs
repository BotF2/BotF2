...
namespace Supremacy.Combat
{
    ...
    public abstract class CombatEngine
    {
        ...
        protected CombatOrder GetCombatOrder(Orbital source)
        {
            ...
            try
            {
                GameLog.Core.Test.DebugFormat("Try Get Order for {0} {1} {2}", source.ObjectID, source.Name, source.Design.Name);
                //if(_orders[source.OwnerID].GetOrder(source) == CombatOrder.)
                    _localOrder = _orders[source.OwnerID].GetOrder(source);
                if (_localOrder != null)
                    GameLog.Core.Test.DebugFormat("Got Order for {0} {1} {2}: -> order = {3}", source.ObjectID, source.Name, source.Design.Name, _orders[source.OwnerID].GetOrder(source));
