...
namespace Supremacy.Combat
{
    public sealed class AutomatedCombatEngine : CombatEngine
    {
        ...
        #region old right side left side Properties ? not connected to CombatUpdate friend / hostile?
        ...
        protected override void ResolveCombatRoundCore()
        {
            ...
            #region The old left right sides / think this is not connected to friend all-hostile assets in CombatUpdate
            ...
            {
                if (firstFriendlyUnit.Item1.Owner == combatent.Item1.Owner || CombatHelper.WillFightAlongside(firstFriendlyUnit.Item1.Owner, combatent.Item1.Owner))
                {
                    FriendlyCombatShips.Add(combatent);
                    FriendlyCombatShips.Randomize();
                }
                if (combatent.Item1.Owner == GetTargetOne(firstFriendlyUnit.Item1.Source)|| combatent.Item1.Owner == GetTargetTwo(firstFriendlyUnit.Item1.Source) || !CombatHelper.AreNotAtWar(combatent.Item1.Owner, firstFriendlyUnit.Item1.Owner))
                {
           ...
           // }
            ...
            // 3 =
            // 0 = EmpireID
            // 1 = Have Total Ship in Battle (uncluding Station?)
            // 2 = 0, meaning 0 ships have fired before battle starts.
            ...
            foreach (int ownerID in ownerIDs.Distinct())
            {
                ...
                {
                    // UPDATE X 25 june 2019 added if == 999 then choose a random target. Also DiplomaticReport needs to change to traits, but currently everyone has trait = compassion
                    ...
                    // UPDATE X 25 june 2019 added if == 999 then choose a random target
                    if ((empiresInBattle[q, 2] == 777 || empiresInBattle[q, 2] == 999) && (civi.DiplomacyReport.Contains("Warlike") || civi.DiplomacyReport.Contains("Hostile")))
                    {
                        while (true)
                        {
                ...
                //}
                GameLog.Core.Combat.DebugFormat("Empire Civ in Battle: {0} FirstTarget = {1} 2nd Target = {2}, q = {3}", empiresInBattle[q, 0], empiresInBattle[q, 1], empiresInBattle[q, 2], q);
                q = q + 1;
            ...
            while (true)
            {
                ...
                GameLog.Core.Combat.DebugFormat("Current Target On or Two? in Main While {0} ", TargetOneORTwo);
                ...
                if (indexOfAttackerEmpires > 11)
                {
                    ...
                    GameLog.Core.Combat.DebugFormat("Current Empire about to , retarted to 0, because last empire was 12"); 
                }
                    // Gamelog
