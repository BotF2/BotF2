...
namespace Supremacy.Scripting.Ast
{
    public class BinaryExpression : Expression
    {
        ...
        public override Expression DoResolve(ParseContext ec)
        {
            ...
            ConstantExpression lc = _left as ConstantExpression;
            ...
            ConstantExpression rc = _right as ConstantExpression;
            ...
            if (rc != null && lc != null)
            {
                int previousErrorCount = ec.CompilerErrorCount;
                ConstantExpression e = ConstantFold.BinaryFold(
                    ec,
                    Operator,
                    lc,
                    rc,
        ...
        {
            return TypeUtils.IsUnsignedInt(type) && value < 0
                ? true
                : type == TypeManager.CoreTypes.SByte && (value >= 0x80 || value < -0x80) ||
        ...
        {
            CastExpression leftCast = Left as CastExpression;
            if (leftCast != null)
            {
            ...
            CastExpression rightCast = Right as CastExpression;
            if (rightCast != null)
            {
        ...
        private static bool IsUnsigned(Type t)
        {
            ...
            return t.IsPointer
                ? true
                : t == TypeManager.CoreTypes.UInt32 ||
        ...
        public override void CloneTo<T>(CloneContext cloneContext, T target)
        {
            ...
            if (!(target is BinaryExpression clone))
            {
                return;
            }
        ...
        public override void Dump(SourceWriter sw, int indentChange)
        {
            ...
            SourceLocation leftEnd = Left.Span.End;
            SourceLocation rightStart = Right.Span.Start;
        ...
        public bool DoBinaryOperatorPromotion(ParseContext ec)
        {
            ...
            if (ltype != int32)
            {
                temp = _left is ConstantExpression c
                    ? c.ConvertImplicitly(int32)
                    : ConvertExpression.MakeImplicitNumericConversion(ec, _left, _left.Type, int32);
            ...
            if (rtype != int32)
            {
                temp = _right is ConstantExpression c
                    ? c.ConvertImplicitly(int32)
                    : ConvertExpression.MakeImplicitNumericConversion(ec, _right, _right.Type, int32);
        ...
        static bool DoNumericPromotion(ParseContext ec, ref Expression primaryExpression, ref Expression secondaryExpression, Type type)
        {
            ...
            if (primaryExpression is ConstantExpression c)
            {
            ...
            {
                Type expressionType = primaryExpression.Type;
                    ...
                    expressionType == TypeManager.CoreTypes.SByte)
                {
                    ...
                    if (type != secondaryExpression.Type)
                    {
                        ...
                        temp = c != null
                            ? c.ConvertImplicitly(type)
                            : ConvertExpression.MakeImplicitNumericConversion(ec, secondaryExpression, secondaryExpression.Type, type);
        ...
        private Expression ResolveOperator(ParseContext ec)
        {
            ...
            {
                // Enums
                bool leftIsEnum = TypeManager.IsEnumType(leftType);
                bool rightIsEnum = TypeManager.IsEnumType(rightType);
        ...
        private BinaryExpression ResolveOperatorEqualityRerefence(ParseContext ec, Type leftType, Type rightType)
        {
            ...
            bool leftIsGenericParameter = TypeManager.IsGenericParameter(leftType);
        ...
        private Expression ResolveOperatorEnum(ParseContext ec, bool leftIsEnum, bool rightIsEnum, Type leftType, Type rightType)
        {
            ...
            else if (leftIsEnum)
            {
                ...
                {
                    if (!(Right is ConstantExpression c) || !c.IsDefaultValue)
                    {
                        return null;
                    }
                }
            ...
            else if (rightIsEnum)
            {
                ...
                {
                    if (!(Left is ConstantExpression c) || !c.IsDefaultValue)
                    {
                        return null;
                    }
                }
            ...
            {
                Type promotedType = leftIsEnum ? Left.Type : Right.Type;

                _ = ConvertExpression.MakeExplicitConversion(
                    ec,
            ...
            return TypeUtils.IsImplicitlyConvertible(leftTemp.Type, leftType) ? expr : null;
        }
        ...
        {
            List<PredefinedOperator> temp = new List<PredefinedOperator>();
            Type boolType = TypeManager.CoreTypes.Boolean;
        ...
        protected virtual Expression ResolveOperatorPredefined(ParseContext ec, PredefinedOperator[] operators, bool primitivesOnly, Type enumType)
        {
            ...
            OperatorFlags operatorFlags = Enum.GetValues(typeof(OperatorFlags))
                .Cast<OperatorFlags>()
            ...
            Expression expr = bestOperator.ConvertResult(ec, this);
            ...
            {
                ConstantExpression rc = Right as ConstantExpression;
                ConstantExpression lc = Left as ConstantExpression;
                ...
                if ((lc != null && lc.IsDefaultValue) || (rc != null && rc.IsDefaultValue))
                {
                    ...
                    //
                    SideEffectConstantExpression sideEffect = (rc == null)
        ...
        protected virtual Expression ResolveUserOperator(ParseContext ec, Type l, Type r)
        {
            ...
            Argument leftArg = new Argument(Left);
            _ = args.Add(leftArg);
        ...
        {
            return t == TypeManager.CoreTypes.Object || t == TypeManager.CoreTypes.String || t == TypeManager.CoreTypes.Delegate || TypeManager.IsDelegateType(t);
        }
        ...
        protected class PredefinedOperator
        {
            ...
            public virtual Expression ConvertResult(ParseContext ec, BinaryExpression b)
            {
                ...
                //
                return Left == TypeManager.CoreTypes.Decimal ? b.ResolveUserOperator(ec, b.Left.Type, b.Right.Type) : b;
            }
            ...
            {
                return TypeManager.IsEqual(Left, lexpr.Type) && TypeManager.IsEqual(Right, rexpr.Type)
                    ? true
                    : TypeUtils.IsImplicitlyConvertible(lexpr.Type, Left, true) &&
            ...
            public PredefinedOperator ResolveBetterOperator(ParseContext ec, PredefinedOperator bestOperator)
            {
                ...
                return result == 0 || result > 2 ? null : result == 1 ? bestOperator : this;
            }
        }

        private class PredefinedStringOperator : PredefinedOperator
        {
        ...
        private class PredefinedShiftOperator : PredefinedOperator
        {
            ...
            public override Expression ConvertResult(ParseContext ec, BinaryExpression b)
            {
                ...
                Expression exprTreeExpr = ConvertExpression.MakeImplicitConversion(ec, b.Right, TypeManager.CoreTypes.Int32, b.Right.Span);

                int rightMask = (Left == TypeManager.CoreTypes.Int32) || (Left == TypeManager.CoreTypes.UInt32) ? 0x1f : 0x3f;
